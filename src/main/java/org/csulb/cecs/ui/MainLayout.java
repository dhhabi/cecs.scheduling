...
public class MainLayout extends VerticalLayout implements ViewDisplay,
		ClickListener, ViewChangeListener {
	...
	public void postConstuct() {
		...
		initMenuBar();
		/*
		 * // Serve the image from the theme Resource res = new
		 * ClassResource("/img/csulb.gif");
		 * 
		 * 
		 * // Display the image without caption Image brandImage = new
		 * Image(null, res);
		 */
	...
	private void initMenuBar() {
		...
		menuItemAdmin.addItem("Edit Sections", new MenuBar.Command() {
		...
		menuItemAdmin.addItem("Download Clara", new MenuBar.Command() {
			@Override
			public void menuSelected(MenuItem selectedItem) {
				// TODO Sections Menu
				UI.getCurrent().getNavigator()
						.navigateTo(ViewToken.CLARADOWNLOAD);
			}
		});
		// Resource admin menuitem
		MenuItem menuItemAdminResources = menuItemAdmin.addItem("Resources",
				null);
	...
	public void showView(View view) {
		...
		if (view instanceof MvpPresenterView) {
			viewContainer.setContent(((MvpPresenterView) view)
					.getViewComponent());
		}
	...
	public void buttonClick(ClickEvent event) {
		...
		UI.getCurrent().getNavigator()
				.navigateTo((String) event.getButton().getData());
	...
	public void onAccessDenied(
			org.vaadin.spring.events.Event<AccessDeniedEvent> event) {
		// TODO Redirect to login,

		if (event.getPayload().getCause() != null) {
			Notification.show("Access is denied.",
					"Service can be invoked only by Admin users.",
					Type.ERROR_MESSAGE);
		} else {
			if (event.getPayload().getViewToken().equals(ViewToken.USER)) {
				Notification.show("Access is denied.",
						"You must sign in before accessing User home.",
						Type.ERROR_MESSAGE);
			} else {
				Notification
						.show("Access is denied.",
								"You must sign in as Admin user before accessing Admin home.",
								Type.ERROR_MESSAGE);
			}
		}
	}
